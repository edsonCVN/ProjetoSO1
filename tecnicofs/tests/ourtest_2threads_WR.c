...
int main() {
    ...
    */
    pthread_t tid[N];
    tfs_write_paramts write_input[N];
    tfs_write_paramts_set(&write_input[0], "/f1", TFS_O_CREAT, input1, SIZE, COUNT);
    tfs_read_paramts read_input[N];
    tfs_read_paramts_set(&read_input[0], "/f1", 0, output1, SIZE, COUNT);

    assert(tfs_init() != -1);

    for(int i = 0; i < N / 2; i++) {
        pthread_create(&tid[i], NULL, tfs_write_tapi, (void*)&write_input[0]);
    }

    for(int i = N / 2; i < N; i++) {
        pthread_create(&tid[i], NULL, tfs_read_tapi, (void*)&read_input[0]);
    }
    ...
    for(int i = 0; i < N / 2; i++) {
        assert(write_input[0].rtn_value == (SIZE*COUNT));
        
    }
    for(int i = N / 2; i < N; i++) {
       assert(read_input[0].rtn_value == (SIZE*COUNT));
    }
