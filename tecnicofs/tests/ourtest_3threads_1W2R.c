...
int main() {
    ...
    assert(pthread_create(&tid[0], NULL, tfs_write_tapi, (void*)&write_input) == 0);

    assert(pthread_join(tid[0], NULL) == 0); // confirmar a necessidade de fazer o join antes de chamar as leituras
    
    for(int i = 1; i < 3; i++) {
        assert(pthread_create(&tid[i], NULL, tfs_read_tapi, (void*)&read_input[i-1]) == 0);
    }
    ...
    assert(strncmp(input1, output1, SIZE) == 0 && strncmp(output1, output2, SIZE) == 0);
